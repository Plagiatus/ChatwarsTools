...
// }
let treasureMultipliers = { monsters: 1, treasures: 1 };
let fountainsInTreasureRouteOnly = true;
...
let allCWNodesWithTreasurePath = new Map();
function findHighestTreasureRoute() {
    ...
    let position = fixStartingPosition(startPosition);
    let startNode = allCWNodesWithTreasurePath.get(vectorToString(position));
    startNode.distance = 0;
    allCWNodesWithTreasurePath.set(vectorToString(position), startNode);
    let remainingNodes = Array.from(structuredClone(allCWNodesWithTreasurePath).values());
    while (remainingNodes.length > 0) {
        remainingNodes.sort(sortNodesByDistance);
        let highestNode = remainingNodes.pop();
        if (highestNode.type === TileType.BOSS)
            continue;
        if (highestNode.type !== TileType.FOUNTAIN && fountainsInTreasureRouteOnly)
            continue;
        // all the connections of the current lowest distance node
        let connections = allCWConnections.get(vectorToString(highestNode.position));
        for (let connection of connections) {
            // calculate the new distance of all the connections
            let targetNode = allCWNodesWithTreasurePath.get(vectorToString(connection.position));
            let newWeight = highestNode.distance + getConnectionTreasureWeight(connection);
...
function getConnectionTreasureWeight(node) {
...
function findTreasureRoute() {
    ...
    let startNode = allCWNodesWithPath.get(vectorToString(startPositionVector));
    ...
    let fullPath = [];
    let atStartPosition = false;
    while (!atStartPosition) {
        let node = allCWNodesWithTreasurePath.get(vectorToString(position));
